...
# residual block - building block of encoder and decoder of residual 2D U-Net
class ResBlock(torch.nn.Module):
    def __init__(self, n_in,n_out):
        ...
        self.BN1 = torch.nn.BatchNorm2d(n_in)
        self.BN2 = torch.nn.BatchNorm2d(n_in)
        self.Conv1 = torch.nn.Conv2d(n_in, n_in, 3, padding='same')
        self.Conv2 = torch.nn.Conv2d(n_in, n_out, 3,padding='same')
        self.IdenMap = torch.nn.Conv2d(n_in, n_out, 1,padding='same')
    def forward(self,x):
...
class ResUNet(torch.nn.Module):
    def __init__(self, n_channels,n_classes,filters = [64,128,256,512]):
        ...
        self.upSmpl1 = torch.nn.ConvTranspose2d(filters[3], filters[2], kernel_size=2,stride=2)
        self.decBlock1 = ResBlock(filters[3],filters[2])
        self.upSmpl2 = torch.nn.ConvTranspose2d(filters[2], filters[1], kernel_size=2,stride=2)
        self.decBlock2 = ResBlock(filters[2],filters[1])
        self.upSmpl3 = torch.nn.ConvTranspose2d(filters[1], filters[0], kernel_size=2,stride=2)
        self.decBlock3 = ResBlock(filters[1],filters[0])
        self.finalConv = torch.nn.Conv2d(filters[0], self.n_classes, kernel_size=1)
    ...
    def forward(self,x):
        ...
        toPad = [diffY // 2, diffY - diffY // 2,diffX // 2, diffX - diffX// 2]
